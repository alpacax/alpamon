...
- **Guard**: under systemd a second transient timer, armed before the swap and firing two minutes after the deadline, restores the previous version if the marker is still present, which covers a build that cannot start at all.
