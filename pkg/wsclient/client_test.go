...
// half the time and Run went on to dial. That dial is a connect to the
// backhaul and a call into a caller's own dial hook, both made after the
// caller asked the client to stop, and the loop cannot catch it: its own
// check runs before the wait, not after.
